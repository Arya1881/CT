import { useStudentTripsMe } from '@/hooks/queries';
...
export function StudentTripsPage() {
  const { data: trips, isLoading } = useStudentTripsMe();

  return (
    <div>
      <PageHeader
        title="Trip History"
        description="Recent trips on your assigned bus route."
      />
      <Card>
        <CardContent className="pt-4">
          {isLoading ? (
            ...
            <EmptyState title="No trips yet" message="Trips will appear here once your bus starts running." />
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
                ...
                {trips.map((t) => (
                  <TableRow key={t.id}>
                    ...
                    <TableCell className="text-right">
                      {t.delayMinutes > 0 ? `${t.delayMinutes} min` : 'On time'}
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
